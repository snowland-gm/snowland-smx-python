...
- [ ] 新增 `pysmx/common/random.py`：基于 `secrets` / `os.urandom` 的 CSPRNG 工具（`random_hex` / `random_bytes`）
- [ ] 重写 `pysmx/SM2/_SM2.py` 的 `get_random_str`，改用 CSPRNG（保持 hex 输出兼容既有调用）
- [ ] `Encrypt` 的临时值 `k` 与 `generate_keypair` 的私钥 `d` 改走 CSPRNG，保留"调用方注入熵"的兼容路径 — `pysmx/SM2/_SM2.py:421,501`
- [ ] 统一 `SM9` / `envelope` 的随机源到 `common/random.py` — `pysmx/SM9/_SM9.py:1351,1358`、`pysmx/extra/envelope.py:96,100`
- [ ] 收敛 `pysmx/backend/_backend.py`：仅注册已实现的 `HashBackend` / SM3 `HMACBackend`，移除未实现的 `CipherBackend` / `HMACBackend` 注册 — `pysmx/backend/_backend.py:31-33`
- [ ] 验收：`bandit -r pysmx` 无 High/Medium；`run_smx_tests.py` 全绿；补充 SM2 随机性 / 已知向量单测

## 阶段二 — 测试与代码质量（P1 / P2）

- [ ] 引入 `unittest discover` 自动发现，替换 `run_smx_tests.py` 手工登记用例 — `run_smx_tests.py:13-19`
...
- [ ] 修复 `pysmx/ecc/fq.py` 裸 `except:` → `except NameError`，删除 py2 兼容壳 — `pysmx/ecc/fq.py:12-16`
- [ ] 清理 `SM4/_SM4.py:269` 空 `TODO`；删除 `modular_power` 冗余递归包装（直接 `pow`）— `pysmx/SM2/_SM2.py:36-50`
- [ ] 验收：lint / 类型 / 安全扫描通过；覆盖率达标

## 阶段三 — 工程化与打包（P4）

- [ ] 迁移到 `pyproject.toml` 的 `[project]`（PEP 621），元数据从 `setup.py` 迁出
- [ ] 修正版本声明：移除 `2.7` / `3.6` / `3.7`，添加 `python_requires=">=3.8"` — `setup.py:58-68`
- [ ] 拆分依赖：`requirements.txt`（运行时）/ `requirements-test.txt` / `requirements-bench.txt`，完善 `extras_require`
...
- [ ] 提供 Cython / C 加速后端（参考 `gmssl-pyx` 模式）；或在文档中明确性能定位 — `benchmark.py`
- [ ] 顶层 facade 统一高层 API（如 `sm2_encrypt` 等），收敛 SM4 多套接口
- [ ] 修正 `pysmx/extra/__init__.py` 导出 `envelope`
