...
def private_key_extract(scheme, master_public, master_secret, identity):
    ...
    user_id = Hash_sm3(str2hexbytes(identity))
...
def sign(master_public, Da, msg):
    ...
    msg_hash = Hash_sm3(str2hexbytes(msg))
...
def generate_session_key(idA, idB, Ra, Rb, D, x, master_public, entity, l):
    ...
    uidA = Hash_sm3(str2hexbytes(idA))
    uidB = Hash_sm3(str2hexbytes(idB))
...
def kem_dem_enc(master_public, identity, message, v):
    ...
    k1 = k[:mbytes]
    k2 = k[mbytes:]
    C2 = [hex_msg[i] ^ k1[i] for i in range(mbytes)]
    hash_input = C2 + k2
    b = bytes(hash_input)
    C3 = Hash_sm3(b)[:int(v // 8)]
    return (C1, C2, C3)
